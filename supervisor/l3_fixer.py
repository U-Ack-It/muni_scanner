"""
L3 Fixer — muni_scanner

Same contract as ouroboros.v2/supervisor/l3_fixer.py.
Project root is muni_scanner/. Tools operate on this codebase.

Auto-apply threshold: confidence >= 0.85.
Restarts the API service (uvicorn) if a fix was applied.
Always writes to the shared incident ledger.
"""
...
API_CMD              = ["uvicorn", "api:app", "--port", "8000"]
...
def _tool_search_code(pattern: str, path: str = ".") -> str:
    ...
    try:
        result = subprocess.run(
            ["grep", "-rn", "--include=*.py", "--include=*.sh", pattern, full_path],
...
def _tool_apply_fix(
    ...
    try:
        ...
        open(full, "w").write(new_content)
        _notifier.send(
            f"🔧 <b>L3 muni_scanner — patch applied</b>\n"
...
def _tool_run_bash(command: str) -> str:
    allowed = ("grep", "tail", "head", "cat", "ps", "pgrep", "ss", "wc", "ls", "find")
    if not any(command.strip().startswith(p) for p in allowed):
        return f"BLOCKED: only read-only commands permitted"
    try:
        result = subprocess.run(
...
def _tool_write_incident(
    ...
    entry = f"""
## {inc_id} | muni_scanner | {datetime.now().strftime('%Y-%m-%d')}
**Title**: {title}
...
TOOL_SCHEMAS = [
    {
        "name": "read_file",
        "description": "Read a file from the muni_scanner project (path relative to project root).",
        "input_schema": {
            "type": "object",
    ...
    {
        "name": "search_code",
        "description": "Grep across muni_scanner Python/shell files.",
        "input_schema": {
            "type": "object",
            "properties": {
                "pattern": {"type": "string"},
    ...
    },
    {
        "name": "apply_fix",
        "description": (
            "Replace old_code with new_code in a file. "
            "old_code must be an exact unique substring. "
            "Auto-applies only if confidence >= 0.85."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    {
        "name": "run_bash",
        "description": "Read-only shell commands (grep, tail, ps, pgrep, ss, ls, find, wc, cat, head).",
        "input_schema": {
            "type": "object",
    ...
    {
        "name": "write_incident",
        "description": "Append a structured incident entry to the shared incident ledger.",
        "input_schema": {
            "type": "object",
            "properties": {
                "inc_id":        {"type": "string"},
...
SYSTEM_PROMPT = """You are the L3 Fixer agent for muni_scanner, a municipal bond anomaly
detection service built on FastAPI.

Your job:
1. Receive an L2 escalation describing a semantic anomaly in the system.
2. Use your tools to read the relevant source files and trace the exact root cause.
3. Produce a precise code fix (old_code → new_code).
4. Call apply_fix with confidence >= 0.85 only when certain the fix is correct.
   If confidence < 0.85, still call apply_fix — it skips the write but logs the proposal.
5. Call write_incident to record the full diagnosis in the incident ledger.

Rules:
- Never guess. Read the actual code first.
- old_code in apply_fix must be an exact substring of the current file.
- Do not restart the API process — that is handled externally.
- Be precise and terse. No padding.
"""
...
def run_l3(escalation: dict) -> dict:
    ...
    user_msg = (
        f"Escalation from L2 Supervisor (muni_scanner):\n\n"
        f"ID:         {escalation['id']}\n"
        ...
        f"Diagnose, fix if confident, write incident to ledger."
    )
    ...
    for _ in range(12):
        ...
        for block in response.content:
            ...
            if block.name == "apply_fix":
                if block.input.get("confidence", 0) >= AUTO_APPLY_THRESHOLD and "OK:" in output:
    ...
    _notifier.send(
        f"{'✅' if fixed else '📋'} <b>L3 muni_scanner complete</b>\n"
        f"<code>{escalation['id']}</code>\n"
        + (f"Patched: {', '.join(f'<code>{f}</code>' for f in applied_fixes)}" if fixed
           else "No auto-fix — see proposal in Telegram above.")
    )
...
def _restart_api():
    """Kill and restart uvicorn api:app --port 8000 after a patch."""
    try:
        subprocess.run(["pkill", "-f", "api:app"], timeout=5)
        import time; time.sleep(2)
        log_path = os.path.join(PROJECT_ROOT, "logs", "api.log")
        subprocess.Popen(
            API_CMD,
            cwd=PROJECT_ROOT,
        ...
        _notifier.send("♻️ <b>muni_scanner API restarted</b> after L3 patch.")
    except Exception as exc:
        _notifier.send(f"⚠️ <b>L3 muni_scanner: restart failed</b> — {exc}")
